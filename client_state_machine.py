...
class ClientSM:
    ...
    def connect_to_game(self):
        msg = json.dumps({"action": "connect_to_game"})
        mysend(self.s, msg)
        response = json.loads(myrecv(self.s))
        if response["status"] == "success":
            return True
    ...
    def proc(self, my_msg, peer_msg):
        ...
        if self.state == S_LOGGEDIN:
            ...
            if len(my_msg) > 0:
                if my_msg == "q":
                    ...
                    peer = my_msg[1:]
                    peer = peer.strip()
                    if self.connect_to_game() == True:
                        self.out_msg += "Start playing! "
                        self.out_msg += "-----------------------------------\n"
                        self.out_msg += print_board()
                        self.state = S_GAME
        ...
        # ==============================================================================
            ...
            if len(my_msg) > 0:
                if (my_msg[0] in ["a", "b", "c"]) and (my_msg[1] in ["0", "1", "2"]):
                    mysend(
                        self.s,
                        json.dumps(
                            {
            ...
            if result["status"] == "invalid":
                self.out_msg += "Invalid move. Try again.\n"
            elif result["status"] == "win":
                self.out_msg += print_board(result["board"])
